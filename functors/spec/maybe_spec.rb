...
RSpec.describe Maybe do
  ...
  context "As a mapp-able container it..." do
    ...
    xit "maps a function over the contained value applying it correctly" do
      expect(Maybe.new(10).fmap(add_ten)).to eq Maybe.new(20)
    end

    xit "wont blow up if we map over a nothing" do
      expect(nothing.fmap(add_ten)).to eq nothing
    end

    xit "hitting a nil in a chain of functions wont blow up" do
  ...
  context "follows the second functor law" do
    xit "composed functions are equal to successive non composed functions" do
      plus_four = ->(x) { x + 4 }
  ...
  context "BONUS ROUND" do
    ...
    xit "functions are partially applied" do
